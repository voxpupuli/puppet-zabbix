  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      when 'RedHat'
        ...
        context "on RedHat #{major} and Zabbix 8.0" do
          let :params do
            {
              zabbix_version: '8.0',
              manage_repo: true
          ...
          it { is_expected.to contain_yumrepo('zabbix').with_baseurl("https://repo.zabbix.com/zabbix/8.0/stable/rhel/#{major}/$basearch/") }
          it { is_expected.to contain_yumrepo('zabbix').with_gpgkey('https://repo.zabbix.com/RPM-GPG-KEY-ZABBIX-B5333005') }
          it { is_expected.to contain_yumrepo('zabbix-nonsupported').with_baseurl("https://repo.zabbix.com/non-supported/rhel/#{major}/$basearch/") }
